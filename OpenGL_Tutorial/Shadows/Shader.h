...
public:
	...
	void SetMatrix(const glm::mat4 mat, const std::string &name)
	{
		matrixID = glGetUniformLocation(shaderProgram, name.c_str());
	...
	void SetLight(const glm::vec4 vec)
	{
		// GLuint lightId = glGetUniformLocation(shaderProgram, "light");

		// std::cout << "Matrix : " << glGetUniformLocation(shaderProgram, "mvp") << std::endl;
		// std::cout << "Light  : " << glGetUniformLocation(shaderProgram, "light") << std::endl;
		// Send our transformation to the currently bound shader,
		// in the "MVP" uniform
		// For each model you render, since the MVP will be different (at least the M part)
		// glUniformMatrix4fv(lightId, 1, GL_FALSE, &mat[0][0]);
		// glUniform4fv(lightId, 1, &vec[0]);
