...
class Renderer
{
	...
	void SetMatrix( const glm::mat4 &model )
	{
		...
		glm::mat4 normal = glm::inverseTranspose(mv);

		simpleShader.SetMatrix( mvp, "mvp" );
		simpleShader.SetMatrix( normal, "normal" );

		simpleShader.SetLight( glm::vec4() );
	}

	void SetLight( const glm::vec4 &vec )
	{
		simpleShader.SetLight( vec );
	}
